...
def calib_recep(progress_callback):
    ...
    pX = []
    Z = []
    recep_z0 =np.loadtxt('recep_z0_points.txt')
    print(recep_z0,recep_z0[0])
    for i in range(len(recep_z0)):
        pX.append(recep_z0[i][0])
        pY.append(recep_z0[i][1])
        Z.append(recep_z0[i][2])

    recep_zN =np.loadtxt('recep_zN_points.txt')
    print(recep_zN,recep_zN[0])
    for i in range(len(recep_zN)):
        pX.append(recep_zN[i][0])
        pY.append(recep_zN[i][1])
        Z.append(recep_zN[i][2])

    
    X=[-270,270 ,-270,270,-90,90,-90,90,0,-270,270 ,-270,270,-90,90,-90,90,0]
    Y = [205,205,-205,-205,68.5,68.5,-68.5,-68.5,0,205,205,-205,-205,68.5,68.5,-68.5,-68.5,0]
    Pt_calib = np.array([
        X,
        Y,
        Z
